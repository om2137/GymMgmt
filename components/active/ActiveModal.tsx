...
// main modal
export default function ActiveModal( {first,middle,last,image,address,phone,birthdate,age,gender,mstatus,id, paid, due, admission, card, paidOn}:Props ) {
...
const handleInvoice = async(e: React.ChangeEvent<any>) => {
  ...
  try{
      ...
      const res = await axios(`${baseUrl}/api/archive/${id}`, {
          method: "PUT",
          headers:{
              "Content-Type": "application/json",
          },
...
// post invoice ended
  ...
  const handleDelete = async() => {
    try{
        const deleteMember = await axios( `${baseUrl}/api/active/${id}` , {
          method: "DELETE",
        });
        router.push('/');
    }catch(err){
